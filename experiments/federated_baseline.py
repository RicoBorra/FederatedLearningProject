...
def get_arguments() -> Any:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog = "note: argument \'--scheduler\' accept different learning rate scheduling choices (\'exp\' or \'step\') followed by decaying factor and decaying period\n\n" \
            ...
            ">>> python3 experiments/script.py --batch_size 512 --learning_rate 0.01 --epochs 5 --rounds 1000 --scheduler step 0.75 3\n\n" \
            "This command executes the experiment using\n" \
            " [+] batch size: 512\n" \
            ...
            ">>> python3 experiments/script.py --niid --batch_size 512 --learning_rate 0.01 --epochs 5 --rounds 500 --scheduler onecycle 0.1 --checkpoint 5 ./saved\n\n" \
            "This command executes the experiment using\n" \
...
if __name__ == '__main__':
    ...
    server = server.Server(
        algorithm = algorithm.FederatedAverage, 
        model = model,
        clients = clients,
        args = args
    )
    ...
    print(f'  [-] learning rate scheduling: {args.scheduler}')
    ...
    wandb.init(
        ...
        name=f"FEMNIST{'_NIID' if args.niid else '_IID'}_S{args.seed}_BS{args.batch_size}_LR{args.learning_rate}_M{args.momentum}_WD{args.weight_decay}_NR{args.rounds}_NE{args.epochs}_LRS{','.join(args.scheduler)}_C{args.selected}_S{args.selection}_R{args.reduction}",
        config={
            'seed': args.seed,
