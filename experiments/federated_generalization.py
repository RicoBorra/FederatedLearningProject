...
def initialize_federated_algorithm_with_configuration(args: Any) -> algorithm.FedAlgorithm:
    ...
    if not args.algorithm or args.algorithm[0] == 'fedavg':
        ...
        algorithm_class = algorithm.FedSr
        # specific fedsr configuration
        config.define('beta_l2', 1e-3 if len(args.algorithm) < 3 else float(args.algorithm[2]))
        config.define('beta_kl', 1e-3 if len(args.algorithm) < 4 else float(args.algorithm[3]))
...
def get_arguments() -> Any:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog = "note: argument \'--decay {variable}\' accept different choices (\'exp\', \'linear\' or \'step\') followed by its options\n\n" \
            "examples:\n\n" \
            ">>> python3 experiments/script.py --validation_domain_angle 75 --batch_size 256 --lr 0.1 --momentum 0.9 --weight_decay 0.0001 --rounds 1000 --epochs 1 --decay lr exp 0.5 --algorithm fedyogi 0.9 0.99 1e-4 1e-2\n\n" \
            ...
            ">>> python3 experiments/script.py --batch_size 512 --lr 0.01 --epochs 5 --rounds 1000 --decay lr step 0.75 3 --algorithm fedsr 512 1e-3 1e-2\n\n" \
            "This command executes the experiment using\n" \
            ...
            ">>> python3 experiments/script.py --validation_domain_angle 30 --niid --batch_size 512 --lr 0.01 --epochs 5 --rounds 500 --algorithm fedavg --selection poc 30\n\n" \
            "This command executes the experiment using\n" \
            ...
            " [+] running local client epoch: 5\n\n" \
            ">>> python3 experiments/script.py --validation_domain_angle 30 --niid --batch_size 512 --lr 0.01 --epochs 5 --rounds 500 --algorithm fedsr 64 1e-2 5e-4 --selection poc 30\n\n" \
            "This command executes the experiment using\n" \
            " [+] validation domain angle: among rotated domains (0, 15, ..., 75) the one of 30 degrees counter clockwise rotation is selected for validation\n" \
...
if __name__ == '__main__':
    ...
    identifier = f"dg_{'niid' if args.niid else 'iid'}{'' if args.validation_domain_angle is None else f'_va{args.validation_domain_angle}'}_s{args.seed}_a{':'.join(args.algorithm)}_r{args.rounds}_e{args.epochs}_c{args.selected}_cs{':'.join(args.selection)}_lr{args.lr}_bs{args.batch_size}_m{args.momentum}_wd{args.weight_decay}_rd{args.reduction}"
