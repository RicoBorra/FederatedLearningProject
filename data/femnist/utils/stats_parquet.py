...
def load_data(name):
    ...
    for f in files:
        ...
        users.extend(data['user'].unique())
        num_samples.extend(data.value_counts(['user'], sort=False).values)
        num_classes.extend(data.groupby(by='user').nunique()['y'])

    num_classes = np.array(num_classes)
    classes_avg = num_classes.mean()
    classes_stddev = num_classes.std()

    return users, num_samples, classes_avg, classes_stddev

def print_dataset_stats(name):
    users, num_samples, classes_avg, classes_stddev = load_data(name)
    num_users = len(users)
    ...
    print(f"num_classes (mean): {classes_avg:.2f}")
    print(f"num_classes (std): {classes_stddev:.2f}")
