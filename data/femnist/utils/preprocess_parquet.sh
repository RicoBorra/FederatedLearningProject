...
VERIFICATION_FILE="" # --verify <fname>, check if parquet files' MD5 matches given digest
...
# TODO: checksum for parquet
# ive done find and replace on json

if [ -n "${VERIFICATION_FILE}" ]; then
    pushd ../$NAME >/dev/null 2>/dev/null
        TMP_FILE=`mktemp /tmp/dir-checksum.XXXXXXX`
        find 'data/' -type f -name '*.parquet' -exec md5sum {} + | sort -k 2 > ${TMP_FILE}
        ...
        if [ $? -ne 0 ]; then
            ...
            echo "Matching parquet files and checksums found!"
        fi
...
if [ "$CONT_SCRIPT" = true ] && [ ! $SAMPLE = "na" ]; then
    if [ -d "data/sampled_data" ] && [ "$(ls -A data/sampled_data)" ]; then
        ...
        if [ $SAMPLE = "iid" ]; then
            LEAF_DATA_META_DIR=${META_DIR} python3 sample_parquet.py $NAMETAG --iid $IUSERTAG $SFRACTAG --seed ${SEED_ARGUMENT}
        else
            LEAF_DATA_META_DIR=${META_DIR} python3 sample_parquet.py $NAMETAG $SFRACTAG --seed ${SEED_ARGUMENT}
        fi
...
if [ "$CONT_SCRIPT" = true ] && [ ! $MINSAMPLES = "na" ]; then
    if [ -d "data/rem_user_data" ] && [ "$(ls -A data/rem_user_data)" ]; then
        ...
        if [ -z $MINSAMPLES ]; then
            python3 remove_users_parquet.py $NAMETAG
        else
            python3 remove_users_parquet.py $NAMETAG --min_samples $MINSAMPLES
        fi
...
if [ "$CONT_SCRIPT" = true ] && [ ! $TRAIN = "na" ]; then
    if [ -d "data/train" ] && [ "$(ls -A data/train)" ]; then
        ...
        if [ -z $TRAIN ]; then
            LEAF_DATA_META_DIR=${META_DIR} python3 split_data_parquet.py $NAMETAG $TFRACTAG --seed ${SEED_ARGUMENT}
        elif [ $TRAIN = "user" ]; then
            LEAF_DATA_META_DIR=${META_DIR} python3 split_data_parquet.py $NAMETAG --by_user $TFRACTAG --seed ${SEED_ARGUMENT}
        elif [ $TRAIN = "sample" ]; then
            LEAF_DATA_META_DIR=${META_DIR} python3 split_data_parquet.py $NAMETAG --by_sample $TFRACTAG --seed ${SEED_ARGUMENT}
        fi
...
# TODO: again checksums

if [ -z "${NO_CHECKSUM}" ]; then
    echo '------------------------------'
    echo "calculating parquet file checksums"
    find 'data/' -type f -name '*.parquet' -exec md5sum {} + | sort -k 2 > ${CHECKSUM_FNAME}
